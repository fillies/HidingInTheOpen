# logistic_fit_and_plots_with_prints_and_logit_understanding.py
...
# Logistic regression helper for general trend
def logistic_reg_trend(x, y):
    """
    Fit logit(p) = β0 + β1 * x using GLM Binomial.
    Here y is a proportion in [0,1].
    """
    eps = 1e-6
...
def main():
    for filename in os.listdir(INPUT_FOLDER):
        ...
        for col in df.columns:
            ...
            try:
                # 1) Logistic regression for general trend in understanding
                beta0, beta1, pval, glm_res = logistic_reg_trend(x, y)
                direction = "decrease" if beta1 < 0 else "increase"

                # 2) 2-parameter logistic curve fit (original)
                ...
                print(
                    f"   logistic_reg: beta0={beta0:.4f} | beta1={beta1:.4f} "
                ...
                # Overlay logistic regression trend as dashed curve
                yhat_logit_dense = expit(beta0 + beta1 * x_dense)
                plt.plot(
                    x_dense, yhat_logit_dense,
                    linestyle="--", alpha=0.8,
