...
OUTPUT_FOLDER = "reconstructed_results_llama"  # where results go
...
def reconstruct_once(text, words_modulated):
    ...
    chat_completion = client.chat.completions.create(
        ...
        model=MODEL,  # Free Llama 3.3 70B
    ...
    if len(chat_completion.choices[0].message.content) != 0:
        if '['in chat_completion.choices[0].message.content and ']' in chat_completion.choices[0].message.content and "is correct" not in chat_completion.choices[0].message.content :
            print((chat_completion.choices[0].message.content or "").strip())
        ...
        print(chat_completion)
        print("refused")
        return str(['Refusal']*len(words_modulated))
...
def analyze_file(filepath):
    ...
    for index_column, distorted_column in enumerate(distorted_columns):
        ...
        for index_row, sentence in enumerate(sentences):
            ...
            predictions = ast.literal_eval(reconstruct_once(sentence, words_modulated).lower().replace("a's","a’s").replace("] \n[", ",").replace("]\n[", ",").replace("] -> [", ",").replace("->",',').replace("n't",'n’t'))[:index_column + 1]
